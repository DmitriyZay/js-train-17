...
function replaceText(word, replacement, text) {
  ...
  const regexp1= new RegExp(word,'g');

 text= (text.replace( regexp1, replacement));
...
function extractTextInParentheses(str) {
  // Створення регулярного виразу з використанням зворотніх посилань 
  //для пошуку тексту в круглих дужках /\((.*?)\)/g.
...
// Завдання 5
/**
 * Функція `findWordOccurrences` знаходить всі входження 
 * заданого слова у рядок з урахуванням ігнорування регістру.
 *
...
function findWordOccurrences(str, word) {
  ...
  // За допомогою циклу whild створюємо ітерацію поки рядок містить 
  //збіги з регулярним виразом, та змінній match присвоюємо збіги
  // Додавання індексу поточного входження слова у масив.
  // Оновлення lastIndex,присвоєюмо йому значення  match.index + 1, 
  //щоб продовжити пошук з наступного символу
  // Повертаємо масив
...
function replaceWordOccurrences(str, word, newWord) {
  // Створюємо регулярний вираз зі словом, використовуючи флаг 'g' 
  //для глобального пошуку всіх входжень.
  // Заміняємо всі входження слова у рядку на нове слово.
  // Повертаємо результат
const reg=new RegExp(word,'g' );
return str.replaceAll(reg, newWord);
}
...
let arr=[];
if ((regex.flags).includes('i')===true){
	arr.push('ignoreCase');
}
